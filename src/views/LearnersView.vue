  ...
  <div class="max-w-4xl mx-auto px-4 py-8 space-y-6">
    ...
    <div v-else class="grid sm:grid-cols-2 gap-4">
      <div v-for="u in learners" :key="u.id" class="card flex items-center gap-4">
        <div class="w-12 h-12 rounded-full bg-brand-subtle text-brand-600 text-lg font-bold flex items-center justify-center shrink-0">
          {{ u.name.charAt(0) }}
        </div>
        <div class="flex-1 min-w-0">
          <p class="font-semibold text-hi">{{ u.name }}</p>
          <p class="text-sm text-mid truncate">{{ u.email }}</p>
        </div>
